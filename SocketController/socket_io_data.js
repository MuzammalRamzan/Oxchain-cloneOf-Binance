...
async function Main() {
    ...
    io.on("connection", async (socket) => {
        socket.on('wallets', async (user_id) => {
            await checkRoomOrJoin('wallets', socket, user_id);
            GetSpotWallet(io.sockets, user_id);
        });
        socket.on('assets', async (user_id) => {
            await checkRoomOrJoin('assets', socket, user_id);
            GetAssets(io.sockets, user_id);
        });
        socket.on('assets_overview', async (user_id) => {
            await checkRoomOrJoin('assets_overview', socket, user_id);
            GetAssetsOverView(io.sockets, user_id);
        });
        socket.on('derivatives', async (user_id) => {
        ...
        socket.on('spot_order_history', async (user_id) => {
            await checkRoomOrJoin('spot_order_history', socket, user_id);
            SpotOrderHistory(io.sockets, user_id);
        });
        socket.on('spot_trade_history', async (user_id) => {
            await checkRoomOrJoin('spot_trade_history', socket, user_id);
            SpotTradeHistory(io.sockets, user_id);
        });
        socket.on('spot_funds', async (user_id) => {
            await checkRoomOrJoin('spot_funds', socket, user_id);
            SpotFunds(io.sockets, user_id);
        });

        socket.on('derivatives_wallet', async (user_id) => {
            await checkRoomOrJoin('derivatives_wallet', socket, user_id);
            DerivativesFunds(io.sockets, user_id);
        });
        /*
        socket.on('margin_cross_balance', async (user_id) => {
            await checkRoomOrJoin('margin_cross_balance',socket, user_id);
            GetCrossWallet( io.sockets, user_id);
        });
        socket.on('margin_isolated_balance', async (user_id) => {
            await checkRoomOrJoin('margin_isolated_balance',socket, user_id);
            GetIsolatedWallet( io.sockets, user_id);
        });
        socket.on('cross_open_orders', async (user_id) => {
            await checkRoomOrJoin('cross_open_orders',socket, user_id);
            CrossOpenOrders( io.sockets, user_id);
        });
        socket.on('cross_trade_history', async (user_id) => {
            await checkRoomOrJoin('cross_trade_history',socket, user_id);
            CrossTradeHistory( io.sockets, user_id);
        });
        socket.on('cross_order_history', async (user_id) => {
            await checkRoomOrJoin('cross_order_history',socket, user_id);
            CrossOrderHistory( io.sockets, user_id);
        });
        socket.on('cross_positions', async (user_id) => {
            await checkRoomOrJoin('cross_positions',socket, user_id);
            CrossPositions( io.sockets, user_id);
        });
        socket.on('cross_funds', async (user_id) => {
            await checkRoomOrJoin('cross_funds',socket, user_id);
            CrossFunds( io.sockets, user_id);
        });
        socket.on('isolated', async (user_id) => {
            await checkRoomOrJoin('isolated',socket, user_id);
            GetIsolatedWallet( io.sockets, user_id);
        });
        socket.on('isolated_open_orders', async (user_id) => {
            await checkRoomOrJoin('isolated_open_orders',socket, user_id);
            IsolatedOpenOrders( io.sockets, user_id);
        });
        socket.on('isolated_trade_history', async (user_id) => {
            await checkRoomOrJoin('isolated_open_orders',socket, user_id);
            IsolatedTradeHistory( io.sockets, user_id);
        });
        socket.on('isolated_order_history', async (user_id) => {
            await checkRoomOrJoin('isolated_open_orders',socket, user_id);
            IsolatedOrderHistory( io.sockets, user_id);
        });

        socket.on('isolated_positions', async (user_id) => {
            await checkRoomOrJoin('isolated_open_orders',socket, user_id);
            IsolatedPositions( io.sockets, user_id);
        });
        socket.on('isolated_funds', async (user_id) => {
            await checkRoomOrJoin('isolated_open_orders',socket, user_id);
            IsolatedFunds( io.sockets, user_id);
        });
        */
        socket.on('future_balance', async (user_id) => {
            await checkRoomOrJoin('future_balance', socket, user_id);
            GetFutureWallet(io.sockets, user_id);
        });
        socket.on('future_positions', async (user_id) => {
            await checkRoomOrJoin('future_positions', socket, user_id);
            FuturePositions(io.sockets, user_id);
        });
        socket.on('future_open_orders', async (user_id) => {
            await checkRoomOrJoin('future_open_orders', socket, user_id);
            FutureOpenOrders(io.sockets, user_id);
        });
        socket.on('future_order_history', async (user_id) => {
            await checkRoomOrJoin('future_order_history', socket, user_id);
            FutureOrderHistory(io.sockets, user_id);
        });
        socket.on('future_trade_history', async (user_id) => {
            await checkRoomOrJoin('future_trade_history', socket, user_id);
            FutureTradeHistory(io.sockets, user_id);
        });
        socket.on('future_transaction_history', async (user_id) => {
            await checkRoomOrJoin('future_transaction_history', socket, user_id);
            FutureTransactionHistory(io.sockets, user_id);
        });
        socket.on('future_assets', async (user_id) => {
            await checkRoomOrJoin('future_assets', socket, user_id);
