...
const getUserId = async function (req, res) {
  ...
  authFile.apiKeyChecker(api_key_result).then((result) => {
    if (result === true) {
      ...
      if (email == null && phone_number == null || email == "" && phone_number == "" || email == " " && phone_number == " ") {

        return res.json({ status: "fail", message: "email_and_phone_number_are_null", showableMessage: "Email and Phone Number are Null" });
      }
      ...
      if (email != null) {
        User.findOne({
          ...
          .then((user) => {
            if (user != null) {
              ...
              let phone = "";
              let email = "";
              if (user["phone_number"] != null && user["phone_number"] != "" && user["phone_number"] != " ") {
                phone = "enabled";
              }
        ...
        if (phone_number != null) {
          ...
          if (country_code == null) {
            return res.json({ status: "fail", message: "country_code_is_null", showableMessage: "Country Code is Null" });
          }
