...
const EditSpotOrder = async (req, res) => {
	...
	let relevantOrder = await Orders.findOne({
		_id: order_id,
		user_id: user_id,
		status: 1,
	});
	...
	if (relevantOrder.type == 'limit') {
		...
		if (relevantOrder.method == 'buy') {
			let balance =
				towallet.amount + relevantOrder.target_price * relevantOrder.amount;
			...
			if (target_price >= price) {
				...
				fromWallet.amount =
					parseFloat(fromWallet.amount) + parseFloat(buyAmount);
				towallet.amount = parseFloat(towallet.amount) - parseFloat(total);

				await fromWallet.save();
				await towallet.save();
				await SetTradeVolumeAndRefProgram({
					order_id: newOrder._id,
					user_id: user_id,
					trade_from: 'spot',
					trade_type: 'buy',
					amount: amount,
					totalUSDT: splitLengthNumber(total),
				});
				...
				towallet.amount =
					splitLengthNumber(towallet.amount) - splitLengthNumber(refund);
				await towallet.save();
				await relevantOrder.save();
			}
		...
		if (stop_limit == null || stop_limit == '')
			return res.json({ status: 'fail', message: 'Invalid parameter' });
		var urlPair = relevantOrder.pair_name.replace('/', '');
		let url = 'http://global.oxhain.com:8542/price?symbol=' + urlPair;
		let result = await axios(url);
		let price = result.data.data.ask;
