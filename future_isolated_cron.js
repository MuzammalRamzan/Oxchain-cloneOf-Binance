...
async function initialize() {
  ...
  let isUpdate = FutureOrder.watch([
    { $match: { operationType: { $in: ["update"] } } },
  ]).on("change", async (data) => {
    orders = await FutureOrder.find(request).exec();
  });
  let isRemove = FutureOrder.watch([
  ...
  let isDelete = FutureOrder.watch([
  ...
  allTickers.onopen = () => {
    allTickers.onmessage = async (data) => {
      ...
      let totalPNL = 0.0;
      let orders = await FutureOrder.find({ status: 0, method: "market", future_type: "isolated" });
